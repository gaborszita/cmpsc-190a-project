...
class GameInfoDataset(Dataset):
  ...
  def __init__(self, data, team_name_mapping, min_vals, max_vals, mean_vals, std_vals):
...
random.shuffle(game_info_detailed_2)

print("data length: " + str(len(game_info_detailed_2)))

split_idx = int(len(game_info_detailed_2) * 0.8)
print(split_idx)

team_name_mapping = TeamNameMapping([elem[1] for elem in game_info_detailed_2])

train_data = game_info_detailed_2[:split_idx]
random.shuffle(train_data)
test_data = game_info_detailed_2[split_idx:]

print("Training data length: " + str(len(train_data)))
print("Test data length: " + str(len(test_data)))

data_rest = torch.tensor([elem[3:] for elem in game_info_detailed_2])
min_vals = data_rest.min(dim=0).values
max_vals = data_rest.max(dim=0).values
mean_vals = data_rest.mean(dim=0)
std_vals = data_rest.std(dim=0)

train_dataset = GameInfoDataset(train_data, team_name_mapping, min_vals, max_vals, mean_vals, std_vals)
test_dataset = GameInfoDataset(test_data, team_name_mapping, min_vals, max_vals, mean_vals, std_vals)
